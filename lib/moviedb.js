...
module.exports = function(api_key, base_url, language) {
  if(api_key) return new MovieDB(api_key, base_url, language);
...
function MovieDB(api_key, base_url, language) {
...
var execMethod = function(type, params, endpoint, fn){
  ...
  var req = request(type, endpoints.base_url + endpoint)
            .query({api_key : this.api_key, language: this.language})
            .set('Accept', 'application/json');
